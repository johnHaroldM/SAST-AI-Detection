...
it('turns a triage decision into labeled training data', function () {
    ...
    expect(
        app(RubixTriageService::class)
            ->trainingReadiness()['false_positive']
    )->toBe(1);
});
...
it('recalculates the rule false-positive rate after triage', function () {
    ...
    $this->actingAs($this->user)
        ->postJson(route('api.findings.triage', $findings->last()), [
...
it('recommends suppressing a rule once its FP rate clears the high threshold', function () {
    ...
    /*
     * Use the real human-triage path rather than populating final_label
     * directly. Rule statistics should be based on attributable labels.
     */
    ...
    // 100% FP over 5 samples clears the 85% suppress threshold.
    expect($rule->total_seen)->toBe(5)
        ->and($rule->total_false_positive)->toBe(5)
        ->and($rule->historical_fp_rate)->toBe(1.0)
...
it('recommends reviewing config in the middle band', function () {
    ...
    $rule = Rule::factory()->create([
        'external_id' => 'php.middling.rule',
    ]);

    $falsePositives = Finding::factory()
        ->count(3)
        ->vectorized()
        ->create([
    ...
    $truePositive = Finding::factory()
        ->vectorized()
        ->create([
            'rule_id' => 'php.middling.rule',
        ]);

    foreach ($falsePositives as $finding) {
        $this->actingAs($this->user)
            ->postJson(route('api.findings.triage', $finding), [
                'corrected_label' => 'false_positive',
            ])
    ...
    $this->actingAs($this->user)
        ->postJson(route('api.findings.triage', $truePositive), [
            'corrected_label' => 'true_positive',
        ])
        ->assertOk();
...
it('leaves a rule unflagged below the sample floor', function () {
    ...
    /*
     * The observed rate is still 100%, but three samples are not enough
     * to recommend suppressing or changing the rule configuration.
     */
    expect($rule->total_seen)->toBe(3)
        ->and($rule->total_false_positive)->toBe(3)
        ->and($rule->historical_fp_rate)->toBe(1.0)
...
it('records a label for every finding in a bulk triage', function () {
    ...
    expect(Finding::whereNotNull('final_label')->count())->toBe(5)
        ...
        ->and(
            app(RubixTriageService::class)
                ->trainingReadiness()['true_positive']
        )->toBe(5);
});
...
it('counts only labels added since the last training run', function () {
    ...
    ModelState::factory()->create([
        'trained_at' => now()->addSecond(),
    ]);
